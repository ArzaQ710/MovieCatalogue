...
open class MovieViewModel : ViewModel() {
    ...
    internal fun setData(language: String) {
        ...
        AndroidNetworking.get("https://api.themoviedb.org/3/movie/popular?api_key=$API_KEY&language=${if (language == "in") "id" else "en"}&page=1")
            ...
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject) {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        ...
                        dataItems.name = jsonObject.optString("title")
                    ...
                    Log.d("DATA", "${data.hasObservers()} and ${data.hasActiveObservers()}")
                }
    ...
    internal fun setSearchData(language: String, query: String) {
        ...
        AndroidNetworking.get("https://api.themoviedb.org/3/search/movie?api_key=$API_KEY&language=$language&query=$query")
            .setPriority(Priority.MEDIUM)
            .build()
            .getAsJSONObject(object : JSONObjectRequestListener {
                override fun onResponse(response: JSONObject) {
    ...
    fun getData(): LiveData<ArrayList<DataModelFilm>> {
        return data
    }
